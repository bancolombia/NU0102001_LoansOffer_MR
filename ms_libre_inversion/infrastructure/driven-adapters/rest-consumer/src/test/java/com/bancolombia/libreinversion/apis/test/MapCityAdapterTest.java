...
public class MapCityAdapterTest {

    private MockWebServer mockWebServer;
    private Mono<List<City>> response;
    private ObjectMapper mapper;
    private Mono<List<City>> responseMono;
    ...
    public void init() throws IOException {
        ...
        attributes.add(Attribute.builder().name("productCode").value("14").build());
        RuleRequest request = RuleRequest.builder()
                .attributes(attributes)
                .productCode("14")
                .ruleName("CiudadPermitida")
                .build();

        List<City> cities = new ArrayList<>();
        cities.add(City.builder()
                .code("CIUDAD_00000001")
                .name("SIN INFORMACIÓN")
                .build());
        response = Mono.just(cities);
        ...
        ruleResponseMono = adapter.ruleValidateIncreaseMemory(request);
        responseMono = adapter.getAllowedCities();
    }
