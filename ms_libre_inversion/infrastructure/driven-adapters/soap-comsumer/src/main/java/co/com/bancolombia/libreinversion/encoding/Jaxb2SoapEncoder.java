...
public class Jaxb2SoapEncoder implements Encoder<Object> {
    ...
    private static final TechLogger log = LoggerFactory.getLog(Jaxb2SoapEncoder.class.getName());
    ...
    private Flux<DataBuffer> encode(Object value, DataBufferFactory bufferFactory) {
    ...
    public static <T> String convetObjectXmlString(T model) {
        try {
    ...
    public static <T> String convetObjectXmlString(T model,boolean xmlDeclaration) {
        try {
            JAXBContext jaxbContext = JAXBContext.newInstance(model.getClass());
            Marshaller marshaller = jaxbContext.createMarshaller();
            marshaller.setProperty("jaxb.encoding", "ISO-8859-1");
            marshaller.setProperty("com.sun.xml.bind.xmlDeclaration", xmlDeclaration);
            marshaller.setProperty(Marshaller.JAXB_FORMATTED_OUTPUT, Boolean.TRUE);
            StringWriter sw = new StringWriter();
            marshaller.marshal(model, sw);
            return sw.toString();
