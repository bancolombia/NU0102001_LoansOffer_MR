...
@Component
public class CustomerCommercialAdapter implements CustomerCommercialGateway {

    @Value("${routesLF.routeCustomerCommercial}")
    private String routeCustomerCommercial;
    ...
    private static final TechLogger logger = LoggerFactory.getLog("CustomerCommercialAdapter");

    public CustomerCommercialAdapter(@Value("${routesLF.urlBase}") String baseUrl) throws SSLException {
    ...
    @ReactiveRedisCacheable(
            cacheName = "customerCommercial", key = "'id_' + #request.data.customerDocumentId", timeout = 300L)
    public Mono<ResponseCustomerCommercial> retrieveCustomerCommercial(
            RequestCustomer request, String messageId) {
        return webClient
                .post()
                .uri(routeCustomerCommercial)
                ...
                .onStatus(HttpStatus::is4xxClientError, response -> Mono.error(new AmortizationInternalException(
                        ExceptionEnum.LI001.name(), ExceptionEnum.LI001.name(),
                        ExceptionEnum.LI001.getMessage(), "", "", messageId)))
                .onStatus(HttpStatus::is5xxServerError, response -> Mono.error(new AmortizationInternalException(
                        ExceptionEnum.LI002.name(), ExceptionEnum.LI002.name(),
                        ExceptionEnum.LI002.getMessage(), "", "", messageId)))
                .bodyToMono(ResponseCustomerCommercial.class)
                .retryWhen(retry(messageId, ExceptionEnum.LI002))
